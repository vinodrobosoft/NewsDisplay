package com.robosoft.newsapp.model.helper
...
object PersistenceHelper {
    ...
    private fun process(newsTop: NewsResponse.NewsTop?,
    onSuccess: Realm.Transaction.OnSuccess? = null,
                        onError: Realm.Transaction.OnError? = null) {
            ...
            object : Realm.Transaction {
                override fun execute(realm: Realm) {
                    ...
                    if(nextId == null)
                    {
                        ...
                        nextId =
                            (realm.where(NewsArticleDetails::class.java).max("id")?.toInt()?.plus(1))
                    }
